...
- **`/insights` route** — Retired. The router redirects `/insights → /` and
  ...
  doughnut chart was not adopted. The standalone `useInsightsStore`
  (`src/stores/insights.ts`) is no longer consumed by any view and can be
  removed in a future cleanup pass. The `src/api/insights.ts` module is
  still in use — `useDashboardStore` calls `fetchEntityDistribution` and
  `fetchMoodDrilldown` from it.
