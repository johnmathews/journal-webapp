...
- **StorylineDetailView** (`/storylines/:id`) — Book-style reader (2026-07-12 redesign): a slim sticky **ChapterToc** (unread dots, derived date ranges) beside the chapters rendered top-to-bottom — one **ChapterReader** per immutable published chapter (cited narrative + addenda + Rename/Mark-unread/Unpublish menu) and a subdued **DraftBlock** last (the growing draft, with Refresh). Scrolling a published chapter into view marks it read; `?chapter=<id>` deep-links. Citation numbering restarts per chapter via `useCitationRegistry`. **StorylineAnchorEditor** provides inline anchor editing with a "Save & refresh" confirm step (`PUT /api/storylines/{id}/anchors`). See [`storylines.md`](./storylines.md) for the full feature reference.
