...
- **AdminLayout / AdminDashboard / AdminServerView** (`/admin/*`) — Admin-only area gated by the router's `requiresAdmin` meta + `useAuthStore.isAdmin`. `AdminLayout` renders the page header and a tab bar (Users / Server) with an embedded `<RouterView/>`. `AdminDashboard` (Users tab, `/admin`) lists all users with stats and toggles `is_active`. `AdminServerView` (Server tab, `/admin/server`) exposes three buttons that hit `POST /api/admin/reload/{ocr-context,transcription-context,mood-dimensions}` to make the server re-read its file-backed config without a restart; each section shows the server's summary response (provider, stack, dimension list, timestamp) and surfaces `ApiRequestError.message` on failure. Backed by `src/api/admin.ts` (typed reload functions); no Pinia store — local component state is enough for one-off operator actions.
